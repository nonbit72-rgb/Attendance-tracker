...
function renderLectures() {
  ...
  /* Show hint when a filter is active */
  const hintEl = document.getElementById('export-hint');
  if (hintEl) {
    hintEl.textContent = filterSubject ? '(filtered — export will match)' : '';
  }
...
/* ==========================================
   CSV IMPORT
   Accepts: subject, date, time, status columns
   - Auto-detects header row (first cell non-numeric)
   - Falls back to today's date if date is missing/invalid
   - Skips rows missing subject; counts them
   - Blocks duplicates via isDuplicate()
   ========================================== */
async function importCSV(input) {
  ...
  try {
    ...
    showImportStatus('Could not read file: ' + e.message, 'error');
    return;
  }
  ...
  if (lines.length === 0) {
    showImportStatus('The CSV file is empty.', 'warn');
    return;
  }
  ...
  let startRow  = 0;
  let colSubject = 0, colDate = 1, colTime = 2, colStatus = 3;

  const firstRow  = parseCSVLine(lines[0]);
  const firstCell = firstRow[0].replace(/"/g, '').trim();

  /* If first cell is non-numeric → treat as header */
  if (firstCell === '' || isNaN(Number(firstCell))) {
    startRow = 1;
    firstRow.forEach((cell, i) => {
      const h = cell.toLowerCase().replace(/[^a-z]/g, '');
      if (h === 'subject' || h === 'name' || h === 'course')       colSubject = i;
      else if (h === 'date')                                         colDate    = i;
      else if (h === 'time')                                         colTime    = i;
      else if (h === 'status' || h === 'attendance' || h === 'att') colStatus  = i;
    });
  }
  ...
  for (let i = startRow; i < lines.length; i++) {
    ...
    /* Extract & trim each field */
    const subject   = (cells[colSubject] || '').replace(/"/g, '').trim();
    const dateRaw   = (cells[colDate]    || '').replace(/"/g, '').trim();
    const timeRaw   = (cells[colTime]    || '').replace(/"/g, '').trim();
    const statusRaw = (cells[colStatus]  || '').replace(/"/g, '').trim();
    ...
    /* Parse time (HH:MM) – optional, empty string if not parseable */
    let time = '';
    if (timeRaw) {
    ...
    /* Parse status – default Present; accept any value as-is if unrecognised */
    let status = 'Present';
    if (statusRaw) {
      const sl = statusRaw.toLowerCase();
      if (sl === 'absent' || sl === 'a' || sl === '0')       status = 'Absent';
      else if (sl === 'present' || sl === 'p' || sl === '1') status = 'Present';
      else status = statusRaw; /* import as-is per spec */
    }

    /* Skip duplicates */
    if (isDuplicate(subject, date, time, null)) { skipped++; continue; }
    ...
    try {
      ...
      /* Update local cache immediately so next isDuplicate check is accurate */
      state.lectures.push({ subject, date, time, status, imageId1: null, imageId2: null });
      imported++;
    } catch (e) {
      skipped++;
    }
  ...
  /* Full refresh from DB so state is clean */
  if (imported > 0) await dataChanged();

  const msg = `✓ Imported ${imported} record${imported !== 1 ? 's' : ''}.` +
              (skipped > 0 ? ` ${skipped} row${skipped !== 1 ? 's' : ''} skipped.` : '');
  showImportStatus(msg, imported > 0 ? 'ok' : 'warn');
}

/** Shows the import result banner */
function showImportStatus(msg, type) {
  const el = document.getElementById('import-status');
  if (!el) return;
  el.textContent = msg;
  el.className = 'import-status ' +
    (type === 'ok' ? 'import-ok' : type === 'error' ? 'import-error' : 'import-warn');
  el.classList.remove('hidden');
}
/* ==========================================
   BUNK CALCULATOR
...
function renderBunkCalc() {
  ...
  if (total === 0) {
    ...
    if (pct >= OVERALL_TARGET) {
      ...
      if (canSkip === 0) {
        badgeClass = 'bunk-safe'; badgeSub = 'Status'; badgeText = 'At limit';
      } else {
        badgeClass = 'bunk-can'; badgeSub = 'Can Skip';
        badgeText = canSkip + (canSkip === 1 ? ' lecture' : ' lectures');
      }
    } else {
      const mustAttend = Math.ceil((OVERALL_TARGET * total - present) / (1 - OVERALL_TARGET));
      badgeClass = 'bunk-must'; badgeSub = 'Must Attend';
  ...
  subjects.forEach(subj => {
    ...
    if (pct >= SUBJECT_TARGET) {
      ...
      rowClass = 'can-bunk'; badgeClass = 'can';
      badgeText = canSkip === 0 ? 'At limit' : '✓ Skip ' + canSkip;
    } else {
      const mustAttend = Math.ceil((SUBJECT_TARGET * s.total - s.present) / (1 - SUBJECT_TARGET));
      rowClass = 'must-attend'; badgeClass = 'must';
    ...
    rowsHTML += `
      <div class="bunk-row ${rowClass}">
        ...
        <span class="bunk-row-pct">${pctInt}%</span>
...
/* ==========================================
   CSV EXPORT
   Exports filtered list (or all) as .csv
   - Commas inside values replaced with semicolons
   - Dates in YYYY-MM-DD (already stored that way)
   - Filename: attendtrack_export_YYYY-MM-DD.csv
   ========================================== */
function exportCSV() {
  const filterSubject = document.getElementById('filter-subject').value;
  let list = filterSubject
  ...
  /* Sort oldest → newest for a readable export */
  list = list.slice().sort((a, b) => {
    const ka = (a.date || '') + 'T' + (a.time || '00:00');
    const kb = (b.date || '') + 'T' + (b.time || '00:00');
  ...
  /* Escape a single CSV field:
     replace internal commas with semicolons, wrap in quotes if needed */
  ...
  /* Build rows */
  const rows = ['subject,date,time,status'];
  list.forEach(l => {
    rows.push([
      csvField(l.subject),
      csvField(l.date  || ''),
  ...
  a.href     = url;
  a.download = `attendtrack_export_${today}.csv`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);

  const label = filterSubject ? `${list.length} filtered records` : `all ${list.length} records`;
